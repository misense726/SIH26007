...
Target `esp32:esp32:esp32c3:CDCOnBoot=cdc` using Espressif Arduino core
3.3.11.
...
Both sensors share GPIO4 SDA and GPIO5 SCL. Read them sequentially with a 5 ms
guard. At boot and recovery, hold both XSHUT pins low, release one sensor,
initialize it at `0x29`, assign and probe its runtime address, then release the
next sensor. Report failed or out-of-range measurements as `-1`.
