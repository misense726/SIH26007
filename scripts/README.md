...
- `start_live_fogsen.ps1` runs MAIN USB and Wi-Fi ingestion together. It finds
  the CP210x USB port, binds the API and Wi-Fi listener to all interfaces, and
  resolves the Pi camera by default. `-NoCamera` starts a sensor-only runtime;
  `-SerialPort COMx` overrides USB discovery. Telemetry defaults to 20 Hz.
...
For a live hardware run, start `start_live_fogsen.ps1` from the repository root,
